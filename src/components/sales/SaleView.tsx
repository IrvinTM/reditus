...
fetch(`${appUrl}/api/sales/sale/${saleId}`)
 ...
 .catch((e)=> console.log(e))
    ...
    if (receiptRef.current) {
      ...
      if (printWindow) {
        ...
        printWindow.document.write(`
          ...
          <html>
            <head>
              <title>Receipt - Sale #${saleData.id}</title>
              <style>
                body {
  ...
  const subtotal = saleData.items.reduce((sum, item) => sum + item.priceAtSale * item.quantity, 0)
            ...
            <div ref={receiptRef}>
              ...
              <div className="receipt-info mb-6 space-y-1 text-sm">
                <div className="flex justify-between">
                  ...
                  <span className="font-mono font-semibold">{saleData.id}</span>
                </div>
                <div className="flex justify-between">
                  <span className="text-muted-foreground">Date:</span>
                  <span className="font-mono">{formatDate(saleData.date)}</span>
                </div>
                <div className="flex justify-between">
                  <span className="text-muted-foreground">Register:</span>
                  <span className="font-mono">{saleData.cashRegisterID}</span>
                </div>
                <div className="flex justify-between">
                  <span className="text-muted-foreground">Customer ID:</span>
                  <span className="font-mono">{saleData.customerID}</span>
                </div>
              </div>
              ...
              <div className="totals space-y-2">
                ...
                <div className="total-row flex justify-between text-sm">
                  ...
                  <span className="font-mono">{formatCurrency(saleData.discount)}</span>
                </div>
                <Separator className="my-3" />
                <div className="total-row total-final flex justify-between text-xl">
                  <span>Total:</span>
                  <span className="font-mono">{formatCurrency(saleData.total)}</span>
                </div>
              </div>
